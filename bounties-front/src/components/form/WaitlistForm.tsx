...
import { waitlistAPI, type WaitlistSubmission } from "../../lib/api/waitlist";
...
export default function WaitlistForm() {
  ...
  const handleSend = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const xHandle = (form.elements.namedItem("xHandle") as HTMLInputElement).value;
      const role = (form.elements.namedItem("role") as HTMLSelectElement).value as 'creator' | 'protocol';

      const waitlistData: WaitlistSubmission = {
        email,
        xHandle,
        role
      };

      await waitlistAPI.submit(waitlistData);
        ...
        <form onSubmit={handleSend} className="flex flex-col items-center gap-6">
          <div className="w-full flex flex-col md:flex-row gap-6">
            ...
            <input
              ...
              className="flex-1 bg-transparent text-white placeholder:text-white/60 px-6 py-4 rounded-full outline-none border border-white disabled:opacity-50"
            />
            <label htmlFor="waitlist-xhandle" className="sr-only">X Handle</label>
            <input
              id="waitlist-xhandle"
              name="xHandle"
              type="text"
          ...
          <div className="w-full">
            <select
              ...
              disabled={isSubmitting}
              defaultValue=""
              className="w-full bg-transparent text-white px-6 py-4 rounded-full outline-none border border-white disabled:opacity-50 [&>option]:bg-gray-800 [&>option]:text-white"
            >
          ...
          <div className="flex flex-row gap-2 sm:gap-4 w-full max-w-md">
            ...
            >
              {isSubmitting ? (
                ...
                <>
                  <span className="cursor-pointer whitespace-nowrap">JOIN WAITLIST</span>
